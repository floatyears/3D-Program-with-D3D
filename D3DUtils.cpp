...
//着色器资源视图定义了一个着色器在渲染期间可以访问的子资源，它可能包含了一个纹理缓冲或者一个纹理
	...
	UINT mipFilter )
{
	...
	//从分离的文件中装载纹理元素，这些纹理不会被GPU使用，它们仅仅被用来从文件装载图像数据。
	UINT size = filenames.size();
	std::vector<ID3D11Texture2D*> srcTex(size);
	for(UINT i = 0; i < size; ++i)
	{
		//D3DX11_IMAGE_LOAD_INFO可选的提供信息来告诉装载APIs如何装载纹理。
	...
	for(UINT texElement =0; texElement < size; ++texElement)
	{
		for(UINT mipLevel = 0; mipLevel < texElementDesc.MipLevels; ++mipLevel)
		{
			...
			//Map函数获取包含在Subresource中的数据的指针，指定一个子资源，需要纹理数组的索引和mip映射等级
	...
	//创建一个着色器资源视图，第一个参数将作为输入，到着色器中，必须用D3D11_BIND_SHADER_RESOURCE创建。
