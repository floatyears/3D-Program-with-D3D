...
class VecAddDemo : public D3DApp
{
public:
...
VecAddDemo::VecAddDemo(HINSTANCE hInstance):D3DApp(hInstance),mOutputBuffer(0), mOutputDebugBuffer(0),
...
void VecAddDemo::DoComputeWork()
{
	...
	{
		Effects::VecAddFX->VecAddTech->GetPassByIndex(i)->Apply(0,md3dImmediateContext);

		md3dImmediateContext->Dispatch(1,1,1);
	}

	//先为compute shader清理绑定的输入纹理
...
void VecAddDemo::BuildBuffersAndViews()
{
	...
	//ShaderResourceView用来给着色器访问SubResources，gpu和cpu资源的相互访问是通过它来完成的。
