...
TreeBillboardApp::TreeBillboardApp(HINSTANCE hInstance):D3DApp(hInstance),mLandVB(0), mLandIB(0), mWavesVB(0), mWavesIB(0), mBoxVB(0), mBoxIB(0), mTreeSpritesVB(0),
...
void TreeBillboardApp::DrawScene()
{
	...
	//
	// DrawTreeSprites() 改变了输入布局和图元拓扑，所以根据下一步要画出的几何图形改变它
	// changes InputLayout and PrimitiveTopology, so change it based on 
	...
	case RenderOptions::TexturesAndFog:
		boxTech = Effects::BasicFX->Light3TexFogTech;
...
void TreeBillboardApp::DrawTreeSprites(CXMMATRIX viewProj)
{
	...
	for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		float blendFactor[4] = {0.0f, 0.3f, 0.0f, 0.0f};
