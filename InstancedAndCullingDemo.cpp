...
class InstancingAndCullApp:public D3DApp
{
public:
	InstancingAndCullApp(HINSTANCE hInstance);
	~InstancingAndCullApp();
				   ...
				   PSTR cmdLine, int showCmd)
{
	...
	InstancingAndCullApp theApp(hInstance);
...
InstancingAndCullApp::InstancingAndCullApp(HINSTANCE hInstance)
...
InstancingAndCullApp::~InstancingAndCullApp()
{
	ReleaseCOM(mSkullVB);
	ReleaseCOM(mSkullIB);
...
bool InstancingAndCullApp::Init()
{
	if(!D3DApp::Init())
		return false;
...
void InstancingAndCullApp::OnResize()
{
	D3DApp::OnResize();
...
void InstancingAndCullApp::UpdateScene(float dt)
{
	...
	outs.precision(6);
	outs<< L"Instancing and Culling Demo" << L"    " << mVisibleObjectCount << L" objects visible out of" << mInstancedData.size();
	mMainWndCaption = outs.str();
	
}

void InstancingAndCullApp::DrawScene()
{
...
void InstancingAndCullApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void InstancingAndCullApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void InstancingAndCullApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void InstancingAndCullApp::BuildSkullGeometryBuffers()
{
...
void InstancingAndCullApp::BuildInstancedBuffer()
{
	const int n = 5;
