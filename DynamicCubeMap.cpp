  ...
  mSkullIndexCount(0), mLightCount(3)
{
	...
	XMStoreFloat4x4(&mGridWorld,XMMatrixMultiply(boxScale,boxOffset));
...
bool DynamicCubeMapApp::Init()
{
	...
	mSky = new Sky(md3dDevice,L"Textures/sunsetcube1024.dds",5.0f);
...
void DynamicCubeMapApp::DrawScene()
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		md3dImmediateContext->ClearDepthStencilView(mDynamicCubeMapDSV,D3D11_CLEAR_DEPTH|D3D11_CLEAR_STENCIL,1,0);

		//将立方体的面绑定为render target
...
void DynamicCubeMapApp::DrawScene(const Camera& camera, bool drawCenterSphere)
{
	...
	if(drawCenterSphere)
	{
		...
		for (UINT i = 0; i < techDesc.Passes; i++)
		{
			...
			activeReflectTech->GetPassByIndex(i)->Apply(0, md3dImmediateContext);
			md3dImmediateContext->DrawIndexed(mSphereIndexCount, mSphereIndexOffset, mSphereVertexOffset);
...
void DynamicCubeMapApp::BuildDynamicCubeMapViews()
{
	...
	// 给cube map 创建一个shader resource view
	...
	// 我们需要一个深度纹理来将场景渲染到cube map中，这个深度纹理和cube map的面有相同的分辨率
