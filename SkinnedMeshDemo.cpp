...
void SkinnedMeshApp::DrawSceneToSsaoNormalDepthMap()
{
	...
	ID3DX11EffectTechnique* animatedTech = Effects::SsaoNormaldDepthFX->NormalDepthSkinnedTech;
	...
	for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		Effects::SsaoNormaldDepthFX->SetWorldViewProj(worldViewProj);
		Effects::SsaoNormaldDepthFX->SetTexTransform(XMMatrixIdentity());
		Effects::SsaoNormaldDepthFX->SetBoneTransforms(
			&mCharacterInstance1.FinalTransforms[0], 
			mCharacterInstance1.FinalTransforms.size());
...
void SkinnedMeshApp::DrawSceneToShadow()
{
