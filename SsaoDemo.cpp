...
{
	BoundingSphere():Center(0.0f,0.0f,0.0f){}
	XMFLOAT3 Center;
	float Radius;
};
  ...
  mLightRotationAngle(0.0f)
{
	...
	XMMATRIX boxOffset = XMMatrixTranslation(0.0f,5.0f,0.0f);
...
void SsaoApp::DrawScene()
{
	...
	//我们已经将场景的深度值保存到Normal/Depth的深度缓冲中。
	//所以我们设置深度比较函数为Equal来防止重写深度值。
...
void SsaoApp::DrawSceneToSsaoNormalDepthMap()
{
	...
	for (int p = 0; p  < techDesc.Passes; p ++)
	{
		...
		Effects::SsaoNormaldDepthFX->SetWorldViewProj(worldViewPorj);
		Effects::SsaoNormaldDepthFX->SetTexTransform(XMMatrixScaling(8.0f, 10.0f, 1.0f));
		...
		Effects::SsaoNormaldDepthFX->SetWorldInvTransposeView(worldInvTransposeView);
		Effects::SsaoNormaldDepthFX->SetWorldViewProj(worldViewPorj);
		Effects::SsaoNormaldDepthFX->SetTexTransform(XMMatrixScaling(2.0f, 1.0f, 1.0f));
		...
		for(int i = 0; i < 10; ++i)
		{
			...
			Effects::SsaoNormaldDepthFX->SetWorldInvTransposeView(worldInvTransposeView);
			Effects::SsaoNormaldDepthFX->SetWorldViewProj(worldViewPorj);
			Effects::SsaoNormaldDepthFX->SetTexTransform(XMMatrixScaling(1.0f, 2.0f, 1.0f));
		...
		for(int i = 0; i < 10; ++i)
		{
			...
			Effects::SsaoNormaldDepthFX->SetWorldInvTransposeView(worldInvTransposeView);
			Effects::SsaoNormaldDepthFX->SetWorldViewProj(worldViewPorj);
			Effects::SsaoNormaldDepthFX->SetTexTransform(XMMatrixIdentity());
...
void SsaoApp::DrawSceneQuad(ID3D11ShaderResourceView* srv)
{
