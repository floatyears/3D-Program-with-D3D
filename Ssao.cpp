...
void Ssao::OnSize(int width, int height, float fovy, float farZ)
{
	...
	mAbmbientMapViewport.TopLeftX = 0.0f;
	mAbmbientMapViewport.TopLeftY = 0.0f;
	mAbmbientMapViewport.Width = width/2.0f;
	mAbmbientMapViewport.Height = height/2.0f;
	mAbmbientMapViewport.MinDepth = 0.0f;
	mAbmbientMapViewport.MaxDepth = 1.0f;
...
void Ssao:: ComputeSsao(const Camera& camera)
{
	...
	//将环境贴图作为渲染对象绑定。可以看到这个pass没有绑定一个depth/stencil buffer，因为它不需要，而且没有depth/stencil buffer，就不会进行深度测试，这正是我们想要的。
	...
	mDC->RSSetViewports(1, &mAbmbientMapViewport);
...
void Ssao::BlurAmientMap(ID3D11ShaderResourceView* inputSRV, ID3D11RenderTargetView* outputRTV, bool horzBlur)
{
	...
	Effects::SsaoBlurFX->SetTexelWidth(1.0f/mAbmbientMapViewport.Width);
	Effects::SsaoBlurFX->SetTexelHeight(1.0f/mAbmbientMapViewport.Height);
	...
	{
		tech->GetPassByIndex(i)->Apply(0,mDC);
		mDC->DrawIndexed(6,0,0);
