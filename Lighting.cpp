...
private:
	float GetHillHeight(float x, float y)const;
	XMFLOAT3 GetHillNormal(float x, float y)const;
...
private:
	...
	ID3DX11EffectVariable* mfxPontLight;
...
LightingApp::LightingApp(HINSTANCE hInstance):D3DApp(hInstance),mLandIB(0),mLandVB(0),mWavesIB(0),mWavesVB(0),mFX(0),mTech(0),mfxWorld(0),mfxWorldInvTranspose(0),mfxDirLight(0),mfxPontLight(0),mfxSpotLight(0),mfxMaterial(0),mfxWorldViewProj(0),mInputLayout(0),mEyePosW(0.0f,0.0f,0.0f),mTheta(1.5f*MathHelper::Pi),mPhi(0.1f*MathHelper::Pi),mRadius(80.0f)
{
	...
	XMStoreFloat4x4(&mWavesWrold,I);
	XMStoreFloat4x4(&mView,I);
	XMStoreFloat4x4(&mProj,I);

	XMMATRIX wavesOffset = XMMatrixTranslation(0.0f,-3.0f,0.0f);
	XMStoreFloat4x4(&mWavesWrold,wavesOffset);
...
void LightingApp::DrawScene()
{
	...
	mfxPontLight->SetRawValue(&mPointLight,0,sizeof(mPointLight));
	...
	for (UINT i = 0; i < techDesc.Passes; i++)
	{
		...
		world = XMLoadFloat4x4(&mWavesWrold);
...
{
	XMFLOAT3 n(-0.03f*z*cosf(0.1f*x)-0.3f*cosf(0.1f*z),1.0f,-0.3f*sinf(0.1f*x)+0.03f*sinf(0.1f*z));
...
void LightingApp::BuildFX()
{
	...
	mfxPontLight = mFX->GetVariableByName("gPointLight");
