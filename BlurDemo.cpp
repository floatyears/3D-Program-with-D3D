...
void BlurApp::DrawScene()
{
	...
	//恢复back buffer，离屏渲染对象将作为compute shader的输入，用来blurring，所以在我们可以用它作为compute shader的输入前，必须把它从OM阶段解绑
	md3dImmediateContext->OMSetRenderTargets(1,renderTargets,mDepthStencilView);

	mBlur.SetGaussianWeight(4.0f);
	mBlur.BlurInPlace(md3dImmediateContext,mOffscreenSRV,mOffscreenUAV,4);

	//将带有模糊纹理的全屏的si分之一绘制到它上面
