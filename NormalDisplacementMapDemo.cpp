...
void NormalDisplacementMapApp::DrawScene()
{
	...
	for (UINT i = 0; i < techDesc.Passes; i++)
	{
		...
		case RenderOptionsDisplacementMap:
			Effects::DisplacementMapFX->SetWorld(world);
			Effects::DisplacementMapFX->SetWorldInvTranspose(worldInvTranspose);
			Effects::DisplacementMapFX->SetWorldViewProj(viewProj);
	...
	//fx会设置曲面细分阶段，但是不会禁掉它们，所以在这里关掉曲面细分。
	...
	for (UINT i = 0; i < techDesc.Passes; i++)
	{
		...
		for (int j = 0; j < 10; j++)
		{
			...
			activeSphereTech->GetPassByIndex(i)->Apply(0,md3dImmediateContext);
	...
	for (UINT i = 0; i < techDesc.Passes; i++)
	{
		...
		activeSkullTech->GetPassByIndex(i)->Apply(0,md3dImmediateContext);
