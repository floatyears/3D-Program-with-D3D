...
private:
	...
	void BuildScreenQuadGeometryBufers();
...
ShadowsApp::ShadowsApp(HINSTANCE hInstance):D3DApp(hInstance), mSky(0), 
  ...
  mLightRotationAngle(0.0f)
{
	...
	//手动的估算场景边界球面，因为我们知道场景如何构造的。网格是最宽的物体，宽是20，深度是30.0f，中心点在世界的中心。
	//一般来说你需要循环每个世界空间中的点位置来计算边界球面。
	...
	// 阴影突刺在增加多边形的斜度的时候会变得更加严重（从光线的角度
...
void ShadowsApp::DrawScene()
{
	...
	Effects::BasicFX->SetShadowMap(mSmap->DeptchMapSRV());
	...
	Effects::DisplacementMapFX->SetShadowMap(mSmap->DeptchMapSRV());
	...
	// 恢复默认的状态，因为SkyFX在effect文件中改变了他们
	md3dImmediateContext->RSSetState(0);
	...
	// 解绑shadow map作为一个着色器输入，因为我们准备在下一帧渲染它。
	// 阴影的力量可以在任何位置，所以清掉所有的位置。
...
void ShadowsApp::DrawSceneToShadowMap()
{
	...
	XMMATRIX proj = XMLoadFloat4x4(&mLighProj);
...
void ShadowsApp::DrawSceneQuad()
{
	...
	for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		Effects::DebugTexFX->SetTexture(mSmap->DeptchMapSRV());
...
void ShadowsApp::BuildShapeGeometryBuffers()
{
	...
	geoGen.CreateCylinder(0.5f, 0.5f, 1.5f, 15, 15, cylinder);
...
void ShadowsApp::BuildScreenQuadGeometryBufers()
{
