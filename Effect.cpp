...
SsaoNormalDepthEffect* Effects::SsaoNormaldDepthFX = 0;
...
void Effects::InitAll(ID3D11Device* device)
{
	...
	SsaoNormaldDepthFX = new SsaoNormalDepthEffect(device, L"FX/SsaoNormalDepth.fx");
}

void Effects::initOther(ID3D11Device* device)
{
	...
	//TessellationFX = new TessellationEffect(device,L"FX/BezierTessellation.fx");
...
VecAddEffect::VecAddEffect(ID3D11Device* device,const std::wstring& filename):Effect(device,filename)
{
...
TessellationEffect::TessellationEffect(ID3D11Device* device,const std::wstring& filename):Effect(device,filename)
{
	...
	FogStart          = mFX->GetVariableByName("gFogStart")->AsScalar();
	FogRange          = mFX->GetVariableByName("gFogRange")->AsScalar();
	DirLight          = mFX->GetVariableByName("gDirLights");
	Mat               = mFX->GetVariableByName("gMaterial");
	DiffuseMap        = mFX->GetVariableByName("gDiffuseMap")->AsShaderResource();
}
	...
	: Effect(device, filename)
{
	...
	World             = mFX->GetVariableByName("gWorld")->AsMatrix();
	WorldInvTranspose = mFX->GetVariableByName("gWorldInvTranspose")->AsMatrix();
	TexTransfom      = mFX->GetVariableByName("gTexTransform")->AsMatrix();
	EyePosW           = mFX->GetVariableByName("gEyePosW")->AsVector();
	FogColor          = mFX->GetVariableByName("gFogColor")->AsVector();
	FogStart          = mFX->GetVariableByName("gFogStart")->AsScalar();
	FogRange          = mFX->GetVariableByName("gFogRange")->AsScalar();
...
ParticleEffect::ParticleEffect(ID3D11Device* device, const std::wstring& filename):Effect(device,filename)
{
