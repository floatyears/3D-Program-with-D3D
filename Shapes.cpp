...
private:
	...
	XMFLOAT4X4 mSpereWorld[10];
	XMFLOAT4X4 mCylWorld[10];
	XMFLOAT4X4 mBoxWorld;
	XMFLOAT4X4 mGridWolrd;
...
ShapesApp::ShapesApp(HINSTANCE hInstance):D3DApp(hInstance),mVB(0),mIB(0),mFX(0),mTech(0),mfxWorldViewProj(0),mInputLayout(0),mWireframeRS(0),mTheta(1.5f*MathHelper::Pi),mPhi(0.1f*MathHelper::Pi),mRadius(15.0f)
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		XMStoreFloat4x4(&mSpereWorld[i*2+0],XMMatrixTranslation(-5.0f,3.5f,-10.0f+i*5.0f));
		XMStoreFloat4x4(&mSpereWorld[i*2+1],XMMatrixTranslation(5.0f,3.5f,-10.0f+i*5.0f));
	}
}
...
void ShapesApp::DrawScene()
{
	...
	for (UINT i = 0; i < techDesc.Passes; i++)
	{
		XMMATRIX world;
		//draw grid
		world = XMLoadFloat4x4(&mGridWolrd);
		...
		for(int i = 0; i < 10; ++i)
		{
			world = XMLoadFloat4x4(&mSpereWorld[i]);
			mfxWorldViewProj->SetMatrix(reinterpret_cast<float*>(&(world*viewProj)));
			mTech->GetPassByIndex(i)->Apply(0, md3dImmediateContext);
			md3dImmediateContext->DrawIndexed(mSphereIndexCount, mSphereIndexOffset, mSphereVertexOffset);
		}
