...
Terrain::Terrain(void):	mQuadPatchVB(0), 
	mQuadPatchIB(0), 
	mLayerMapArraySRV(0), 
	mBlendMapSRV(0), 
	mHeightMapSRV(0),
...
{
	//HeightmapWidt代表width方向的像素点，乘以格子的宽度，就得到实际的宽度
...
{
	//将坐标从地形空间变换到格子空间，地形空间的点是在中心，而格子所在的空间是在左上角
...
void Terrain::Init(ID3D11Device* device, ID3D11DeviceContext* dc, const InitInfo& initInfo)
{
	...
	//将高度贴图分割为份，以此每一份都有一个每份格子数（每一列或者每一排的格子数）
	//因为地形是根据传入的参数来设置大小
...
void Terrain::LoadHeightMap()
{
	...
	//将数组数据复制到一个浮点数组并且扩大它。
	mHeightmap.resize(mInfo.HeightmapHeight*mInfo.HeightmapHeight,0);
